...
// POST /api/categories/:id/keywords — 키워드 추가 (ADMIN 전용)
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  if (session.user.role !== 'ADMIN') {
    return NextResponse.json({ error: '관리자만 키워드를 추가할 수 있습니다.' }, { status: 403 })
  }
  ...
  const category = await prisma.category.findFirst({ where: { id: params.id } })
  if (!category) {
